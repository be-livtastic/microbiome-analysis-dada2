...
# These objects should exist if you've run single end pipeline
...
# ---- DIAGNOSTIC CHECK 9: SAMPLE RETENTION AFTER FILTERING ----
setdiff(surviving_samples, rownames(filtering_stats)) |> length() -> num_retained_samples
cat("=== CHECK 9: Samples Retained After Filtering ===\n")
cat("Total samples:", length(forward_reads), "\n")
common_samples <- intersect(surviving_samples, rownames(filtering_stats)) 


# ---- DIAGNOSTIC CHECK 10: FILTERING STATISTICS STRUCTURE ----
dim(tracking_table)
colnames(filtering_stats)
str(filtering_stats)
# Check that filtering_stats has expected columns
length(sapply(dada_forward[surviving_samples], count_unique_reads)) 
length(sapply(dada_reverse[surviving_samples], count_unique_reads))
length(sapply(mergers[surviving_samples], count_unique_reads))
str(sapply(mergers[surviving_samples], count_unique_reads))
