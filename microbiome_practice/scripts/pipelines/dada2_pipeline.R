...
raw_dir <- here::here("data", "raw", "NCBI_SRA_fastq") # or "fastq"
...
# Windows requires multithread = FALSE for all DADA2 steps
...
if (pipeline_mode == "standard") {
  ...
  trunc_len <- c(150L, 130L) # Truncate at positions where quality drops below Q25
...
if (pipeline_mode == "cutadapt") {
  ...
  # ---- OR Define cutadapt path explicitly (avoids PATH issues) ----
  cutadapt_path <- "/home/belivtastic/.local/bin/cutadapt"

  # ---- 3. Verify cutadapt works in WSL ----
  cutadapt_version <- tryCatch(
    system2(
      "wsl",
      args = c(cutadapt_path, "--version"),
      stdout = TRUE,
      stderr = TRUE
    ),
  ...
  if (length(cutadapt_version) == 0L) {
    stop("cutadapt not accessible via WSL at: ", cutadapt_path)
  }
  ...
  # ---- 6. Sanity check: input files must exist ----
  stopifnot(all(file.exists(n_filtered_forward)))
  stopifnot(all(file.exists(n_filtered_reverse)))

  # ---- 7. Primer reverse complements ----
  FWD.RC <- dada2:::rc(FWD_primer)
  REV.RC <- dada2:::rc(REV_primer)

  # ---- 8. Cutadapt flags ----
  R1.flags <- c("-g", FWD_primer, "-a", REV.RC)
  R2.flags <- c("-G", REV_primer, "-A", FWD.RC)

  # ---- 9. Convert ALL paths to WSL format ----
  ...
  # ---- 10. Run cutadapt ----
  cutadapt_logs <- vector("list", length(sample_names))
...
saveRDS(tracking_table, file.path(filtered_dir, "Paired_end_tracking_table.rds"))
...
# The SILVA database needs ~4–6GB RAM to load and run.
# If needed, start a fresh R session, load dada2, RDS file, then run taxonomy.

taxonomy <- assignTaxonomy(
  ASV_paired_end_table,
  reference_fasta,
  multithread = use_multithread
)
...
saveRDS(taxonomy, file.path(filtered_dir, "Paired_end_taxonomy.rds"))
...
cat("  1. ASV_paired_end_table.rds               - abundance of each ASV per sample\n")
cat("  2. Paired_end_taxonomy.rds                - taxonomic assignments (Kingdom to Species)\n")
